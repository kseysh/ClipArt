...
String image_query = "SELECT * FROM image_table"; 

//쿼리문 작성
rs = stmt.executeQuery(query);
image_rs = stmt.executeQuery(image_query);
...
<body>
	...
	<div class="main">
					...
					<%			
						...
						<div class="list_container">
							...
							<form action="search.jsp">
								...
								if(count == 0){ 
								%>
									<span class="list_delete_button" style = "color:black;" onclick="delete_classification(<%= rs.getInt("idx") %>, '<%= rs.getString("classification_title") %>')">X</span>  </span>
								<% }else{ %>
									<span class="list_delete_button"  onclick="alert('삭제하고자 하는 분류에 클립아트가 존재하여 삭제할 수 없습니다.')">X</span>	</span>
									<%} %>
		...
		<section class="thumbnails" style="margin: 20px 50px 50px 50px;">
			...
			%>
				<img src="<%=image_source%>" alt="<%=image_rs.getString("originalFileName") %>">
